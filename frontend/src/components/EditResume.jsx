...
import { AlertCircle, ArrowLeft, Check, Download, Loader, Save, Trash2 } from 'react-feather'
...
const EditResume = () => {
  ...
  try {
    const response = await axios.get(`http://localhost:4000/api/resume/${resumeId}`, {
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      const uploadResponse = await axios.put(`http://localhost:4000/api/resume/${resumeId}`,
        {withCredentials: true},
        formData,
        {
    ...
    try {
      setIsLoading(true)
      await axios.put(`http://localhost:4000/api/resume/${resumeId}`,
    {
      ...resumeData,
  ...
  return (
    <DashboardLayout>
        ...
        actionBtnIcon={
          isDownloading ? (
            <Loader size={16} className="animation-spin" />
          ) : 
            downloadSuccess ? (
